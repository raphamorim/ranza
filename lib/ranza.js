...
Ranza.prototype.watch = function(path, install) {
    _.sentinel('/', {watch: true},function(files){
        console.log(_.manager(files));
    })
}

Ranza.prototype.exec = function() {
    _.sentinel('/', {watch: false}, function(files, root){
        var requires = _.manager(files);
        var validRequires = _.cleaner(requires);
        _.compare(root, validRequires);
    })
}

Ranza.prototype.install = function(development) {
    var dev = development || false;
    _.sentinel('/', {watch: false}, function(files, root){
        var requires = _.manager(files);
        var validRequires = _.cleaner(requires);
        _.installer(root, validRequires, dev);
    })
}
