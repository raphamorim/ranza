...
Ranza.prototype.status = function(fn) {
    var self = this;
    return r.sentinel('/').spread(function(files, root){
        var searcher = r.searcher(files),
            requires = r.formater(searcher['requires']);
        return r.compare(root, requires, {debug: self.debug, where: searcher.where}).spread(function(diffs, unused, reqs, log) {
            if (self.moduleMode) {
                var status = {
                    undefinedUsed: diffs, 
